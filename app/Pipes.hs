module Pipes 
    ( Pipe, PipeTile, PositionTable
    , tickrate
    , imageForPipeTile
    , imageForSTPositionTable
    , randomPipe
    , randomPipeStream
    , pipesMain ) where
...
randomPipeStream :: DisplayRegion -> IO (IO Image)
randomPipeStream dbounds = do
    seed    <- RS.randomIO
    rpipe   <- return . randomPipe =<< R.getStdGen
    drawPipeIOSequence seed dbounds rpipe
